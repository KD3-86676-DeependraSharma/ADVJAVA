...
public class CategoryDaoImpl implements CategoryDao {
	...
	public Category getCategoryAndPostDetails(String categoryName) {
		Category category = null;
//	1	String jpql = "select c from Category c where c.categoryName = :nm";
//		String jpql = "select c from Category c join fetch c.blogPosts  where c.categoryName = :nm";//it give the no value exception when blog is not in the category it not give the only category
...
//	1		category.getBlogPosts().size();//select - posts
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
